...
test("raw_request POSTs the body to the given relative path with Bearer auth", async () => {
  const { tools, calls, restore } = harness();
  try {
    const res = await tools.raw_request({
      path: "b2b/v1/pricelists/get",
      body: { pricelist_ids: ["pl-1"] },
    });
    assert.equal(res.isError, undefined);
    assert.equal(calls[0].method, "POST");
    assert.equal(calls[0].url, "https://api.retailtech.yango.com/b2b/v1/pricelists/get");
    assert.equal(calls[0].auth, "Bearer TKN");
    assert.deepEqual(calls[0].body, { pricelist_ids: ["pl-1"] });
  } finally {
    restore();
  }
});

test("raw_request defaults the body to an empty JSON object", async () => {
  const { tools, calls, restore } = harness();
  try {
    await tools.raw_request({ path: "b2b/v1/stores/get" });
